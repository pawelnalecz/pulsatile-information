...
class Chain:
    def __init__(self, parameters, cache_directory=Path('.'), working_directory=Path('.')): 
        ...
        self.important_parameters: Dict[str, set[str]] = {}
        self.steps = []
        self.step_dict: Dict[str, Chain] = {}
        self.cached_files = {}
    ...
    def step(self, module, useCache='if_up_to_date'):
        ...
        if useCache == 'if_present':
            time_fun(step.perform)(**self.parameters) if not self.files_exist(step.output_files) else print(f'Using cached output from \'{ step.full_step_name}\'')
        elif useCache == 'if_up_to_date':
            time_fun(step.perform)(**self.parameters) if not self.are_output_files_up_to_date(step) else print(f'Using cached output from \'{ step.full_step_name }\'')
        else:
            time_fun(step.perform)(**self.parameters)
        return self
    ...
    def add_file(self, as_filetype, file, additional_parameters=[], extension='.pkl.gz'):
        self.parameters = {**self.parameters, '_file_for_' + as_filetype : file}
        FileAdder(self, as_filetypes={as_filetype: extension}, external_files={as_filetype: os.path.relpath(file, self.cache_directory_path)}, additional_parameters=additional_parameters)
        return self

    def add_files(self, as_filetypes, filenames, additional_parameters=[], extension='.pkl.gz'):
        ...
        FileAdder(self,
            ...
            external_files={as_filetype: os.path.relpath(filename, self.cache_directory_path) for as_filetype,filename in zip(as_filetypes, filenames)}, additional_parameters=additional_parameters)
        return self
...
class AbstractStep(ABC):
    ...
    def __init__(self, chain: Chain, custom_filenames = {}) -> None:
        ...
        assert self.input_files is not None, f'Step \'{self.full_step_name}\': Input files must be specified'
        assert self.output_files is not None, f'Step \'{self.full_step_name}\': Output files must be specified'
        assert self.required_parameters is not None, f'Step \'{self.full_step_name}\': Required must be specified'
        assert self.step_name is not None, f'Step \'{self.full_step_name}\': Required must be specified'
        ...
        # print(self.important_parameters)
...
class FileAdder(AbstractStep):
    ...
    def __init__(self, chain: Chain, as_filetypes=[], external_files = {}, additional_parameters=[], predecessors=[]) -> None:
        ...
        for key,val in external_files.items():
            print(f'Added file {val} as {key}')
